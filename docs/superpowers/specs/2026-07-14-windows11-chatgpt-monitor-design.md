...
在保留 macOS 版本核心信息结构的基础上，为 Windows 11 提供原生桌面监控器。应用常驻系统托盘，在屏幕顶部中央显示紧凑胶囊；点击后展开周额度、每日活动、项目分析和统计总览四页数据。
...
- 第三页按项目展示 7 天、30 天和全部历史 Token 排名，并支持悬停明细。
- 第四页使用 2×2 卡片显示累计 Token、峰值 Token、最长任务时长、当前/最长连续天数，禁止文字裁切。
...
- 四页数据没有遮挡，项目分析按钮和排行可准确命中，统计总览完整显示“当前/最长连续”。
