...
- 面板尺寸为 `420 × 320`。四页统一使用 32 点页头、主内容卡和底部指标卡；GitHub 页保留完整贡献热力图和两列三行仓库卡片。不新增侧栏、浏览器外框或独立窗口。
...
页面只保留一个全宽主按钮。剪贴板中已有 `github_pat_` 或 `ghp_` 令牌时，点击按钮直接安全绑定；否则打开 GitHub 令牌创建页。授权成功后关闭弹窗并刷新本页；用户取消授权或授权失败时仍停留在未绑定状态，并在弹窗中提供简短、可重试的错误提示。
...
贡献热力图必须通过单个异步 `Canvas` 绘制，不为 371 个贡献格创建独立 SwiftUI 子视图。未绑定背景不得使用实时高斯模糊或系统材质，避免进入第 4 页时触发大量布局和离屏合成。

分页容器每次只创建当前页，转场期间最多保留进入页和离开页，不得移动包含四页内容的超宽视图。进入 GitHub 页后等待 220 毫秒再读取 Keychain 和刷新网络数据；已有缓存应在转场前显示，Keychain 读取必须在主线程之外执行。
